<?php
/**
 * Tronque une chaine avec le nombre de caractère max
 *
 * @string       string -> Chaine à tronquer
 * @max_length   int    -> Nombre de caratère max
 * @wordsafe     bool   -> Ne pas couper le mot ?
 * @add_ellipsis bool   -> Ajouter ...
 * @return  string
 */
...
function truncate($string, $max_length, $wordsafe = true, $add_ellipsis = true, $min_wordsafe_length = 1) {
    $ellipsis = '';
 ...
 * Reset body classes
 */
add_filter('body_class', 'reset_body_classes');
function reset_body_classes($classes)
{
    $classes = array();
...
/**
 * Ajouter les liens parents de la page courante
 * @param [type] $postId [description]
 */
function breacrumbAddParents(&$breadcrumb, $postId = null)
{   
    ...
    if (!empty($parents)) {
        ...
        foreach ($parents as $parentId) {
            ...
            $breadcrumb[] = '<a href="' . get_permalink($post->ID) . '" itemprop="url">' . get_the_title($post->ID) . '</a>';
            unset($post);
        }
    }
}
 ...
 * @param  array  $menusId -> Liste des menus à prendre en compte (par défaut menu principal)
 * @return HTML
 */
function renderSiteMap($menusId = array(2))
{
    $out = null;
    ...
    if (!empty($routing)) {
        $out .= '<ul>';
        foreach ($routing as $routeName => $routeData) {
            $routingId = routing_get_id($routeName);
            $routingOptions = routing_get_options($routeName);
            $show = (isset($routingOptions['sitemap']) && $routingOptions['sitemap'] === false) ? false : true;

            if ($show & ($routingId != $currentPageId)) {
                $out .= '<li><a href="' . routing_get($routeName) .'">' . get_the_title($routingId) . '</a></li>';
            }
        }
        $out .= '</ul>';
    }
