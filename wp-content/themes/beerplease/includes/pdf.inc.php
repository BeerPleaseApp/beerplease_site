...
function calameo_render( $tags )
{
	...
	$html = '<div style="' . ( empty($attributes['styles']) ? 'text-align: center; width:' . $attributes['width'] . $attributes['widthUnit'] . '; margin: 30px auto; padding:50px 0 50px 0;' : $attributes['styles'] ) . '">';

	$id = 'calameo-viewer-' . $attributes['code'] . '-' . time() . '-' . rand(1000,9999);

	$html .= '<iframe src="' . $viewer_url . '?' . $flashvars . '" width="600" height="400" style="width:600px!important; height:400px!important; background-color:#eeeeee!important; border:none!important; frameborder="0" scrolling="no" allowtransparency allowfullscreen></iframe>';
